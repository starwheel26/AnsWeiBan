...
def saveData(data):
    ...
    with open("questionData.json", "w", encoding="utf8") as file:
...
def main():
    ...
    data = loadData()
    if data == 0:
        print("未找到数据。请把要导入试卷数据以UTF-8保存为data.json")
    else:
        importData = getPaperDetails(data)
        for k in importData.keys():
            v = importData[k]
            if k in questionsBank.keys():
                md5Str = md5(str(v))
                if md5Str != md5(str(questionsBank[k])):
                    questionsBank[k] = v
                    updateNum += 1
        ...
        print(
            f"导入现有外部数据{len(importData)}条\n共有{dataAfterLength}条数据，本次新增{updateQuantity}条，更新{updateNum}条。"
        )
